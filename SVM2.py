...
def main():
    ...
    with open("train_data.json", "r", encoding="utf-8") as train:
        ...
        skipped = 0
        for entry in train:
            if total >= args.max_train_entries:
                break
            total += 1
            try:
    ...
    if args.technique == "bert":
        ...
        with open("svm.pickle", "wb") as f:
            pickle.dump(
                (clf_stars, clf_useful_l1, clf_funny_l1, clf_cool_l1, vectorizer), f
            )
        with open("svm.pickle", "rb") as f:
            clf_stars, clf_useful_l1, clf_funny_l1, clf_cool_l1, vectorizer = (
                pickle.load(f)
            )
